...
QUOTA UNLIMITED ON
    USERS;
GRANT CONNECT, RESOURCE TO C##fasedos;
...
CREATE SEQUENCE resena_id_seq START WITH 1 INCREMENT BY 1;
CREATE SEQUENCE reporte_id_usuario_seq START WITH 1 INCREMENT BY 1;
CREATE SEQUENCE reporte_id_empresa_seq START WITH 1 INCREMENT BY 1
...
CREATE TABLE USUARIO (
    ...
    dpi VARCHAR(500) CONSTRAINT usuario_dpi_nn NOT NULL,
    foto VARCHAR(500) CONSTRAINT usuario_foto_nn NOT NULL
);
...
CREATE TABLE EMPRESA (
    ...
    activo VARCHAR(10) CONSTRAINT empresa_activo_ck CHECK (activo IN ('activo','inactivo')),
    foto VARCHAR(500) CONSTRAINT empresa_foto_nn NOT NULL
);
...
CREATE TABLE PUESTO_TRABAJO (
    ...
    area VARCHAR2(200) CONSTRAINT puesto_area_nn NOT NULL,
    activo VARCHAR(10) CONSTRAINT puesto_activo_ck CHECK (activo IN ('activo','inactivo')),
    prioridad VARCHAR(1) CONSTRAINT puesto_prioridad_ck CHECK (prioridad IN ('1','0')),
    editado VARCHAR(1) CONSTRAINT puesto_editado_ck CHECK (editado IN ('1','0'))
);
...
CREATE TABLE REPORTE_USUARIO ( -- Son los reportes que se le hicieron a los usuarios
    id INTEGER DEFAULT reporte_id_usuario_seq.NEXTVAL PRIMARY KEY,
    id_reportante INTEGER CONSTRAINT id_reportante_reporte_usuario_nn NOT NULL,
    id_reportado INTEGER CONSTRAINT id_reportado_reporte_usuario_nn NOT NULL,
    categoria VARCHAR2(50) CONSTRAINT categoria_reporte_usuario_nn NOT NULL,
    motivo VARCHAR2(255) CONSTRAINT motivo_reporte_usuario_nn NOT NULL,
    fecha_reporte TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_reporte_empresa FOREIGN KEY (id_reportante) REFERENCES EMPRESA(id),
    CONSTRAINT fk_reporte_usuario FOREIGN KEY (id_reportado) REFERENCES USUARIO(id)
    
);

INSERT INTO REPORTE_USUARIO(id_reportante, id_reportado, categoria, motivo)
VALUES (1,53,'Mala actitud', 'Mostró prepotencia');
COMMIT;

CREATE TABLE REPORTE_EMPRESA ( -- Son los reportes que se le hicieron a las empresas
    id INTEGER DEFAULT reporte_id_empresa_seq.NEXTVAL PRIMARY KEY,
    id_reportante INTEGER CONSTRAINT id_reportante_reporte_empresa_nn NOT NULL,
    id_reportado INTEGER CONSTRAINT id_reportado_reporte_empresa_nn NOT NULL,
    categoria VARCHAR2(50) CONSTRAINT categoria_reporte_empresa_nn NOT NULL,
    motivo VARCHAR2(255) CONSTRAINT motivo_reporte_empresa_nn NOT NULL,
    fecha_reporte TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_reporte_usuario FOREIGN KEY (id_reportante) REFERENCES USUARIO(id),
    CONSTRAINT fk_reporte_empresa FOREIGN KEY (id_reportado) REFERENCES EMPRESA(id)
);

INSERT INTO REPORTE_EMPRESA (id_reportante, id_reportado, categoria, motivo)
VALUES (53, 1, 'Comportamiento inapropiado', 'Mala atención en sus instalaciones.');
COMMIT;

CREATE TABLE RESENAS (
    id_resena INTEGER DEFAULT resena_id_seq.NEXTVAL PRIMARY KEY,
    id_usuario INTEGER NOT NULL,
    id_empresa INTEGER NOT NULL,
    calificacion INTEGER CONSTRAINT resena_calificacion_ck CHECK (calificacion BETWEEN 1 AND 5),
    comentario VARCHAR2(1000),
    fecha_resena TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_resena_usuario FOREIGN KEY (id_usuario) REFERENCES USUARIO(id),
    CONSTRAINT fk_resena_empresa FOREIGN KEY (id_empresa) REFERENCES EMPRESA(id)
);

SELECT * FROM USUARIO;

SELECT id FROM USUARIO ORDER BY id;
SELECT id FROM USUARIO WHERE id > 52 ORDER BY id;

DELETE FROM USUARIO WHERE id > 52;
COMMIT;
